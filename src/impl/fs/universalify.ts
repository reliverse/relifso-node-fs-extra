import { type CustomPromisify, promisify } from "node:util";

// https://www.reddit.com/r/typescript/comments/10ibwi4/get_last_item_type_of_a_tuple/j5dogh8/
type LastElement<Arr extends any[]> = Arr extends [...any[], infer B]
  ? B
  : never;

type CallbackArgs<T extends (...args: any) => any> = LastElement<
  Parameters<T>
> extends (err: any, ...cbArgs: infer CbArgs) => void
  ? CbArgs
  : never;
...
export function universalify<
  TFunction extends CustomPromisify<TCustom> & ((...args: any) => any),
  TCustom extends (...args: any) => any,
>(
  fn: TFunction & CustomPromisify<TCustom>,
  fallbackMultiResultObjectCreator: (
    ...args: CallbackArgs<TFunction>
  ) => Awaited<ReturnType<TCustom>>,
): TFunction & TCustom;
export function universalify<
  A extends unknown[],
...
export function universalify<
  TFunction extends CustomPromisify<TCustom>,
  TCustom extends (...args: any) => any,
>(fn: TFunction & CustomPromisify<TCustom>): TFunction & TCustom;
export function universalify<
  A extends unknown[],
  X extends (...args: any[]) => any,
  R,
...
>(
  ...
  const promisified = fallbackMultiResultObjectCreator
    ? promisifyFallback(fn, fallbackMultiResultObjectCreator)
    : promisify(fn);

  const f = (...args: ([...A, (err: any, result: R) => void][0] | A[0])[]) => {
...
function promisifyFallback<T extends any[], TResults extends any[], TFallback>(
  fn: (
    ...args: [...args: T, callback: (err: any, ...results: TResults) => void]
  ) => void,
