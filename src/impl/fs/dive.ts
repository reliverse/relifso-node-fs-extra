...
const DEFAULT_OPTS: Required<Pick<DiveOptions, 'recursive' | 'all' | 'files'>> =
  { recursive: true, all: true, files: true };
...
async function _diveHelper(
  directory: string,
  action: DiveActionPromise,
  options: DiveOptions = {},
): Promise<void> {
...
export function dive(
  ...
  const action = (options === o1 ? o2 : o1) as
    | DiveActionCallback
    | DiveActionPromise;
  ...
  void _diveWorkerCallback(directory, action as DiveActionCallback, opts).finally(
    complete,
  );
}
