...
import { utimesMillisSync } from "~/libs/core/core-impl/lib/util/utimes.js";
...
function onLink(destStat, src, dest, opts) {
  ...
  if (!destStat) {
    ...
    try {
      ...
      if (
        /** @type {NodeJS.ErrnoException} */ (err).code === "EINVAL" ||
        /** @type {NodeJS.ErrnoException} */ (err).code === "UNKNOWN"
      )
        return symlinkSync(resolvedSrc, dest);
      throw err;
    }
