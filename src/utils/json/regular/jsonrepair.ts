...
import {
  ...
  removeAtIndex,
  stripLastOccurrence,
} from "~/utils/json/helpers/stringUtils.js";
...
export function jsonrepair(text: string): string {
  ...
  function parseString(stopAtDelimiter = false, stopAtIndex = -1): boolean {
    ...
    if (isQuote(charAt(i))) {
      ...
      const isEndQuote = isDoubleQuote(charAt(i))
        ? isDoubleQuote
        : isSingleQuote(charAt(i))
          ? isSingleQuote
          : isSingleQuoteLike(charAt(i))
            ? isSingleQuoteLike
            : isDoubleQuoteLike;
  ...
  function parseUnquotedString(isKey: boolean) {
    ...
    while (
      i < text.length &&
      !isUnquotedStringDelimiter(charAt(i)) &&
      !isQuote(charAt(i)) &&
      (!isKey || charAt(i) !== ":")
    ) {
      i++;
