...
function createCore(options?: { chunkSize: number; bufferSize: number }) {
  const chunks: string[] = [];
  const transform = fs.jsonrepairCore({
...
describe("stream", () => {
  test("should create and pipe a jsonrepair transform", async () => {
    ...
    const output = input.pipe(fs.jsonrepairTransform());
  ...
  test("should configure chunk size", async () => {
    ...
    const output = input.pipe(fs.jsonrepairTransform({ chunkSize: 4 }));
    const result = await streamToChunks(output);
  ...
  test("should configure buffer size, should throw error", async () => {
    return new Promise<void>((resolve) => {
      ...
      const output = input.pipe(fs.jsonrepairTransform({ chunkSize: 4, bufferSize: 2 }));
      input.on("error", (err) => {
...
function testInputBuffer(text: string) {
  const buffer = fs.createInputBuffer();
  buffer.push(text);
...
function testOutputBuffer(options?: { chunkSize: number; bufferSize: number }) {
  const chunks: string[] = [];
  const buffer = fs.createOutputBuffer({
    write: (chunk) => {
      chunks.push(chunk);
    },
...
const implementations = [
  { name: "regular", jsonrepair: fs.jsonRepairRegular },
...
describe.each(implementations)("jsonrepair [$name]", ({ jsonrepair }) => {
  describe("parse valid JSON", () => {
    ...
    test("supports escaped unicode characters in a string", () => {
      ...
      expect(jsonrepair('"\\ud83d\\ude00"')).toBe('"\\ud83d\\ude00"');
      expect(jsonrepair('"\\u0439\\u043d\\u0444\\u043e\\u0440\\u043c\\u0430\\u0446\\u0438\\u044f"')).toBe(
        '"\\u0439\\u043d\\u0444\\u043e\\u0440\\u043c\\u0430\\u0446\\u0438\\u044f"',
      );
    });
  ...
  describe("repair invalid JSON", () => {
    ...
    test("should strip JSONP notation", () => {
      ...
      expect(() => console.log({ output: jsonrepair("callback {}") })).toThrow(
        new fs.JSONRepairError('Unexpected character "{"', 9),
      );
    });
  ...
  test("should throw an exception in case of non-repairable issues", () => {
    ...
    }).toThrow(new fs.JSONRepairError("Unexpected end of json string", 0));

    expect(() => {
      console.log({ output: jsonrepair('{"a",') });
    }).toThrow(new fs.JSONRepairError("Colon expected", 4));

    expect(() => {
      console.log({ output: jsonrepair("{:2}") });
    }).toThrow(new fs.JSONRepairError("Object key expected", 1));

    expect(() => {
      console.log({ output: jsonrepair('{"a":2}{}') });
    }).toThrow(new fs.JSONRepairError('Unexpected character "{"', 7));

    expect(() => {
      console.log({ output: jsonrepair('{"a" ]') });
    }).toThrow(new fs.JSONRepairError("Colon expected", 5));

    expect(() => {
      console.log({ output: jsonrepair('{"a":2}foo') });
    }).toThrow(new fs.JSONRepairError('Unexpected character "f"', 7));

    expect(() => {
      console.log({ output: jsonrepair("foo [") });
    }).toThrow(new fs.JSONRepairError('Unexpected character "["', 4));

    expect(() => {
      console.log({ output: jsonrepair('"\\u26"') });
    }).toThrow(new fs.JSONRepairError('Invalid unicode character "\\u26""', 1));

    expect(() => {
      console.log({ output: jsonrepair('"\\uZ000"') });
    }).toThrow(new fs.JSONRepairError('Invalid unicode character "\\uZ000"', 1));
    ...
    }).toThrow(new fs.JSONRepairError('Invalid character "\\u0000"', 4));
...
function createStreamingRepairWrapper(): (input: string) => string {
  return function jsonrepair(text: string): string {
    ...
    const { transform, flush } = fs.jsonrepairCore({
      onData: (chunk) => {
        output += chunk;
      },
