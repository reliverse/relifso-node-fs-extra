import type { JsonReadOptions, JsonWriteOptions } from "deprecated/types.js";
// json.ts
import type { JsonOutputOptions } from "fs-extra";
import type { ReadCallback, WriteCallback } from "jsonfile";

import * as fs from "deprecated/impl/fs/index.js";
import { universalify } from "deprecated/impl/fs/universalify.js";
import { outputFile, outputFileSync } from "deprecated/impl/lib/output-file/index.js";
...
const stripBom = (content: string | Buffer): string =>
  Buffer.isBuffer(content) ? content.toString("utf8").replace(/^\uFEFF/, "") : content.replace(/^\uFEFF/, "");

const stringify = (
  obj: unknown,
  { EOL = "\n", finalEOL = true, replacer, spaces }: Extract<JsonWriteOptions, object>,
): string => {
...
export function readJson<T>(file: fs.PathOrFileDescriptor, options?: JsonReadOptions): Promise<T>;
export function readJson(file: fs.PathOrFileDescriptor, o1?: JsonReadOptions | ReadCallback, o2?: ReadCallback): any {
...
export function readJsonSync<T = any>(file: fs.PathOrFileDescriptor, options: JsonReadOptions = {}): T | null {
  if (typeof options === "string") options = { encoding: options };
...
async function _writeJson(file: fs.PathOrFileDescriptor, obj: unknown, options: JsonWriteOptions = {}): Promise<void> {
  if (typeof options === "string") options = { encoding: options };
...
export function writeJson(file: fs.PathOrFileDescriptor, obj: unknown, callback: WriteCallback): void;
export function writeJson(file: fs.PathOrFileDescriptor, obj: unknown, options?: JsonWriteOptions): Promise<void>;
export function writeJson(
  file: fs.PathOrFileDescriptor,
  obj: unknown,
...
export function writeJsonSync(file: fs.PathOrFileDescriptor, obj: unknown, options: JsonWriteOptions = {}): void {
  if (typeof options === "string") options = { encoding: options };
...
export function outputJson(file: string, data: unknown, options?: JsonOutputOptions): Promise<void>;
export function outputJson(file: string, data: unknown, options: JsonOutputOptions, callback: fs.NoParamCallback): void;
export function outputJson(file: string, data: unknown, callback: fs.NoParamCallback): void;
export function outputJson(
  ...
  return new Promise<void>((res, rej) => {
    outputFile(file, stringify(data, options), options, (err) => (err ? rej(err) : res()));
  });
}

/* ---------------------------- outputJsonSync ------------------------------ */

export function outputJsonSync(file: string, data: unknown, options: JsonOutputOptions = {}): void {
