...
const _writeFileStreamHelper = fromPromise(
  async (path: fs.PathOrFileDescriptor, data: NodeJS.ReadableStream, options?: fs.WriteFileOptions) => {
      ...
      return options
        ? typeof options === "string"
          ...
          : {
              ...
              mode: typeof options.mode === "string" ? Number.parseInt(options.mode, 8) : options.mode,
              flush: options.flush,
            }
        : undefined;
    }
...
export function writeFile(
  ...
  const callback: fs.NoParamCallback | undefined =
    optionalCallback ?? (typeof optionsOrCallback === "function" ? optionsOrCallback : undefined);
  ...
  return backingWriteFile(
    ...
    optionsOrCallback as fs.WriteFileOptions,
    optionalCallback,
  );
}
