import { copySync } from "deprecated/impl/lib/copy/index.js";
import { mkdirsSync } from "deprecated/impl/lib/mkdirs/index.js";
import { removeSync } from "deprecated/impl/lib/remove/index.js";
import { checkParentPathsSync, checkPathsSync } from "deprecated/impl/lib/util/stat.js";
// move-sync.ts
...
export function moveSync(src: string, dest: string, options: MoveOptions = {}): void {
  ...
  const { srcStat, isChangingCase = false } = checkPathsSync(src, dest, "move", options);
...
function moveAcrossDevices(src: string, dest: string, overwrite: boolean): void {
  copySync(src, dest, {
    overwrite,
